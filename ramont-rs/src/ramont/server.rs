...
impl EventDaemon {
    ...
    fn start_daemon(daemon: EventDaemon) -> Arc<Mutex<EventDaemon>> {
        let daemon = Arc::new(Mutex::new(daemon));

        let thread_daemon = daemon.clone();

        let thread_handle = thread::spawn(move || loop {
            let daemon = thread_daemon.lock().unwrap();
            let msg = daemon.receiver.recv().unwrap_or_else(|err| {
            ...
            if Message::Terminate == daemon.notify(msg) {
                println!("Shutting down EventDaemon.");
                break;
            }
        });

        daemon.lock().unwrap().worker = Some(thread_handle);
